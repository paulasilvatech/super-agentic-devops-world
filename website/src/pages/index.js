...
/* ===== PIXEL IMAGE HELPER ===== */
function PixelImg({ src, alt, size = 64 }) {
    const url = useBaseUrl(src);
    return (<img src={url} alt={alt} width={size} height={size} className={styles.pixelImage} loading="lazy"/>);
}
/* ===== HUD ===== */
function HUD() {
...
/* ===== ? BLOCK ===== */
function QuestionBlock({ onClick, hit }) {
    return (<div className={`q-block ${hit ? 'q-block--hit' : ''}`} onClick={onClick} title="Hit me!">
      {hit ? '' : '?'}
    </div>);
}
/* ===== HERO ===== */
function Hero() {
    ...
    return (<header className="hero--game">
      ...
      <div className="hero__content">
        ...
        <div className="question-blocks">
          {hits.map((h, i) => (<QuestionBlock key={i} hit={h} onClick={() => hitBlock(i)}/>))}
        </div>
...
function Stats() {
    ...
    return (<section className="stats-row">
      {items.map((s, i) => (<ScrollReveal key={i} delay={i * 0.1}>
          <div className="stat">
...
function Features() {
    return (<section className="features-section">
      <div className="container">
        ...
        <div className="row">
          {features.map((f, i) => (<ScrollReveal key={i} delay={i * 0.08} direction={i % 2 === 0 ? 'left' : 'right'}>
...
function WorldMap() {
    ...
    return (<section className="world-map">
      <div className="container">
        ...
        <div className={styles.progressBar}>
          <div className={styles.progressTrack}>
            {worlds.map((w) => (<div key={w.num} className={`${styles.progressDot} ${progressDotWorldClasses[w.num - 1]} ${hoveredWorld === w.num ? styles.progressDotActive : ''}`} title={w.theme}/>))}
          </div>
        </div>

        <div className="row world-path">
          {worlds.map((w) => (<div key={w.num} className={`col col--3 ${styles.worldColumn}`}>
              <Link to={w.link} className={styles.worldCardLink} onMouseEnter={() => setHoveredWorld(w.num)} onMouseLeave={() => setHoveredWorld(null)}>
...
function QuickStart() {
    return (<section className="quickstart">
      <div className="container">
        <Heading as="h2" className={styles.sectionTitleTight}>
          Quick Start
        </Heading>
        ...
        <div className={`row ${styles.centeredRow}`}>
          <div className="col col--6">
            {quickLinks.map((q, i) => (<Link key={i} to={q.to} className="pipe-link">
...
function Banner() {
    ...
    return (<section className="banner-section">
      <div className="container">
        <img src={bannerSrc} alt="Super Agentic DevOps World — Made for Sofia" className={`banner-image ${styles.bannerImage}`} loading="lazy"/>
...
export default function Home() {
    return (<Layout title="Press START to Learn" description="The complete guide to Agentic DevOps explained through Super Mario World analogies">
      <Hero />
      <main>
        <Stats />
        <Features />
